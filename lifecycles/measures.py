...
import lifecycles.flow_analysis as fa
__all__ = [
    "facet_unicity",
    "facet_identity",
    "facet_outflow",
...
def facets(lc,target,direction):
    ...
    facets = fa._analyze_one_struct(lc.get_group(target), reference_sets)
    return facets

def event_weights(lc,target,direction):
    names = fa.backward_event_names() if direction == "-" else fa.forward_event_names()
    fscores= facets(lc,target,direction)
    res = fa._compute_event_scores(fscores)
    return dict(zip(names, res))

def event(lc,target,direction=["+","-"]):
    """
