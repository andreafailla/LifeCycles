...
def _null_model(branch, reference, num_permutations):
...
def validated_flow(
    ...
    """
    Compare the flows with null models. Each branch of each flow is compared with a null model of the same size.
    ...
    for name, subset in flow.items():
        null_model = _null_model(subset, reference, num_permutations)[name]
        if _is_significant(len(subset), null_model, significance_level):
            validated[name] = subset
    return validated


def all_validated_flows(
    lc: LifeCycle,
    direction: str,
    min_branch_size=1,
    ...
    """
    Compare all flows with null models. See validated_flow for details.
    ...
    for target, flow in lc.all_flows(direction, min_branch_size).items():
        validated[target] = validated_flow(
            lc, target, direction, min_branch_size, num_permutations, significance_level
        )
    return validated
